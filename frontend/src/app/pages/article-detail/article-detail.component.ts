...
export class ArticleDetailComponent implements OnInit {
  ...
  comments: Comment[] = [];
  newComment: string = '';
  translatedTitle: string | null = null;  // Holds the translated text
  translatedBody: string | null = null;  // Holds the translated text
  translatedComments: { [key: number]: string } = {}; // To store translated comments
  constructor(
  ...
  loadArticle(id: number): void {
    ...
    .pipe(
      map((article: Article) => this.imageProcessingService.createImages(article))
    )
    .subscribe(
      (article: Article) => {
  ...
  disable() {
    this.articleService.disableArticle(this.article).subscribe(
      response => {
        this.snackBar.open("Article has been disabled successfully", "Ok", { duration: 3000 });
        this.router.navigate(['/list-article']);

      }, (error) =>
      this.snackBar.open("Failed to disable article. Please try again." + error.error.message, "Ok", { duration: 3000 }));
  }

// Method to translate the article
translateArticle(): void {
  // Combine title and body into a single string, separated by a newline
  const textToTranslate = `${this.article.title} --- ${this.article.body}`;

  console.info("Combine title and body:",textToTranslate)

  // Send the combined text to the translation service
  this.translationService.translate(textToTranslate).subscribe(
    (response) => {
      // Assuming the response contains the translated text
      const translatedTextParts = response.translation_text.split('---'); // Split the translated text back into title and body
      
      // Update the article with the translated title and body
      this.translatedTitle = translatedTextParts[0]; // Set the translated title
      this.translatedBody = translatedTextParts[1]; // Set the translated body

      console.log('Translated title:', this.translatedTitle);
      console.log('Translated body:', this.translatedBody);
      this.snackBar.open('Article translated successfully!', 'Ok', { duration: 3000 });
    },
    (error) => {
      this.snackBar.open('Failed to translate article. Please try again.', 'Ok', { duration: 3000 });
  ...
  translateComment(comment: any): void {
    this.translationService.translate(comment.content).subscribe(
      (response) => {
        
        console.log('Translation response for: ', comment.id);
        console.log("response:",response);

        this.translatedComments[comment.id] = response.translation_text; // Adjust based on actual response structure
        console.log("all translated:",this.translatedComments)
      },
      (error) => {
        console.error('Translation error:', error);
      }
